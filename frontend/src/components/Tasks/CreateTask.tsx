...
import {
  RiAddLine, RiCalendarLine, RiFlagLine,
  RiPriceTag3Line, RiArrowUpSLine, RiArrowDownSLine,
} from "react-icons/ri";

const PRIORITIES = [
  { value: "high",   label: "High",   color: "text-red-500",   bg: "bg-red-50 dark:bg-red-900/20",   border: "border-red-300 dark:border-red-700" },
  { value: "medium", label: "Medium", color: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-900/20", border: "border-amber-300 dark:border-amber-700" },
  { value: "low",    label: "Low",    color: "text-green-500", bg: "bg-green-50 dark:bg-green-900/20", border: "border-green-300 dark:border-green-700" },
];
...
const CreateTask: React.FC = () => {
  ...
  const reset = () => { setTitle(""); setDesc(""); setPriority("medium"); setDueDate(""); setCategory(""); setTags(""); setShowExtra(false); setOpen(false); };
    ...
    <div className="card p-4 fade-up">
      <form onSubmit={handleSubmit}>
          ...
          {PRIORITIES.map(p => (
            <button key={p.value} type="button"
              ...
              className={clsx(
                ...
                priority === p.value ? `${p.bg} ${p.border} ${p.color}` : "border-gray-200 dark:border-gray-700 text-gray-400 hover:border-gray-300"
              )}>
        ...
        <div className="ml-8 mb-3 flex items-center gap-3">
          ...
          <input type="date" className="input py-1 text-sm w-auto" value={dueDate} onChange={e => setDueDate(e.target.value)} />
        </div>
        ...
        <button type="button" onClick={() => setShowExtra(v => !v)}
          ...
          {showExtra ? <RiArrowUpSLine size={14} /> : <RiArrowDownSLine size={14} />}
        ...
        {showExtra && (
          <div className="ml-8 space-y-2 mb-3">
            <div className="flex items-center gap-2">
              <RiPriceTag3Line size={14} className="text-gray-400" />
              <input className="input py-1 text-sm flex-1" placeholder="Category" value={category} onChange={e => setCategory(e.target.value)} />
            </div>
            <div className="flex items-center gap-2">
              <RiPriceTag3Line size={14} className="text-gray-400" />
              <input className="input py-1 text-sm flex-1" placeholder="Tags (comma separated)" value={tags} onChange={e => setTags(e.target.value)} />
            </div>
          </div>
        )}
        ...
        <div className="flex items-center justify-end gap-2 mt-1">
          ...
          <button type="submit" disabled={!title.trim() || loading} className="btn-primary text-sm py-1.5 px-4 disabled:opacity-50">
